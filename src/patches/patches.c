...
struct Patches Patches_Init(void) {
  ...
  if (D2_GameVersion_GetRunning() < D2_GameVersion_k1_07Beta) {
    return;
  }

  patches.music_patches = MusicPatches_Init();
  patches.ui_patches = UiPatches_Init();
...
void Patches_Deinit(struct Patches* patches) {
  ...
  UiPatches_Deinit(&patches->ui_patches);
  MusicPatches_Deinit(&patches->music_patches);
}
...
void Patches_Apply(struct Patches* patches) {
  ...
  MusicPatches_Apply(&patches->music_patches);
  UiPatches_Apply(&patches->ui_patches);
}
